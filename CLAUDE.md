...
- **`apps/app` (`@cortxt/app`) → app.cortxt.io.** Den inloggade arbetsytan — **destinationen** för en fullvärdig, multi-publik produktyta (en motor, roll-formade vyer: orientera/dirigera, kund-modellering, lärande, samarbete; beslut Rikard 2026-06-22). Växer tunn → full genom roll-lägen, inte ombyggnad. **Ytor:** `/` (**Min arbetsyta** — `Workspace.jsx`, placeholder), `/cockpit` (**Cockpit** — `Cockpit.jsx`: sammanvävd portföljöversikt via `fetchCommandCenter` — **Drift**-rad (deploy-/infra-hälsa, `state.infra`), sitrep, **Vertikaler**-sektion (kort per vertikal: fas X/N + beslut + nästa drag, länkar till per-projekt-vyn), uppdrag/orders), `/vertikal/:slug` (**Per-projekt-vy** — `Vertical.jsx`: **Arkitektur** = ELK-nodgraf via `fetchGraph(slug)` + `@cortxt/graph`, **lazy-laddad** route så reactflow/elkjs code-splittas; + roadmap-faser/öppna beslut via `fetchVertical`), `/verktyg` (**Verktyg** — launcher). En **cockpit-IA** som *ersatte* Min arbetsyta byggdes och **förkastades 2026-06-20** (personlig ≠ verktygs-launcher) — slutsatsen var att portfölj-/health-rollup hör i en **egen** yta; `/cockpit` är nu just den egna ytan (slopar inte Min arbetsyta). **Arbetsytans innehåll är PARKERAT:** `Workspace` är placeholder; "Dina modeller" ska INTE visa hela katalogen (28 interna system) — öppen fråga om produkter/ventures (`productsOnly`) eller ett per-modell "pinnat"-fält i CNS. Auth-placeholder (`src/lib/auth.js`) + **CNS-klient (`src/lib/cns.js`, levande söm — inte längre stub)**. Den tunga grafen är parkerad i `_experiments`.
...
- **Sömmen (`src/lib/cns.js`):** `fetchModels({productsOnly,domains})` → `GET /api/nodes` (normaliserar till camelCase, läser `is_product` från backend med venture-domän som fallback); `fetchDecisionModel(slug)` → `GET /api/node/<slug>/full`; `fetchModelIssues(slug)` → `GET /api/issues?node=`; `fetchCommandCenter()` → `GET /api/command-center` (den sammanvävda lägesbilden från `command_center_state()` — missions/sitrep/orders + `infra` (deploy-hälsa) + `verticals[]` (per-vertikal läge/roadmap/nästa steg); renderas av `pages/Cockpit.jsx` via `hooks/useCommandCenter.js`); `fetchVertical(slug)` → `GET /api/vertical/<slug>` (roadmap-detalj: recept-faser + status/epics + öppna beslut, för `pages/Vertical.jsx`); `fetchGraph(slug)` → `GET /api/nodes?domain=<slug>` **rått** (ej normaliserat — grafen behöver part_of/feeds/depends_on/kind), matar `@cortxt/graph`. Hooks `useModels`/`useDecisionModel` i `src/lib/useModels.js` wrappar med loading/error.
- **Konnektivitet:** `apps/app/vercel.json` proxar `/api/* → Railway` (same-origin i prod). Lokalt: en **Vite dev-proxy** i `apps/app/vite.config.js` (`server.proxy['/api'] → Railway`) speglar prod-rewriten så `cns.js` kör same-origin utan CORS. (Det dokumenterade `VITE_CNS_API_BASE`-direktanropet funkar INTE lokalt — Railway-backenden skickar ingen `Access-Control-Allow-Origin`, så webbläsaren blockerar cross-origin-fetchen; env-varen finns kvar som override men föredra proxyn.) **OBS: Railway redeployar bara vid push till main** — nya ventures/`url_live` i `catalog.yaml`, och nya endpoints som `/api/command-center`, syns på riktiga app.cortxt.io först efter redeploy.
...
- `packages/graph` (`@cortxt/graph`) — **delad nodgraf-komponent** (käll-JSX, som `@cortxt/ui`): lean fetch-fri `<NodeGraph nodes />` (ELK layered-layout + ReactFlow; part_of/feeds/depends_on → nesting/kanter; kind-färg + hälso-prick). Egen ELK-config, ej dashboardens semantic-zoom-tyngd. Konsumeras av `apps/app` per-projekt-vyn (lazy). Deps: `elkjs`, `reactflow`. `optimizeDeps.exclude` i appens vite.config.
...
- Spec först. Bygg inte om det som funkar. **Återanvänd `@cortxt/ui`-primitiver** — hårdkoda inte färger/komponenter per app. App:en byggs ut mot fullvärdig (se ovan); lägg tung återanvändbar logik (hooks/api) i `packages/` när den ska delas med dashboarden, inte duplicerad.
