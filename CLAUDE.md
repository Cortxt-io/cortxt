...
- **`apps/app` (`@cortxt/app`) → app.cortxt.io.** Tunn inloggad arbetsyta, **två ytor**: `/` (**Min arbetsyta** — `Workspace.jsx`: personlig yta med Dina modeller / Senaste exports / Pågående analyser) och `/verktyg` (**Verktyg** — launcher till vertikalerna). En **cockpit-IA** (`/` Översikt med `PortfolioPulse` + `/portfolj` + `/modell/:slug`, instrumentrader `ReadoutRow`/`ReadoutList`) byggdes men **förkastades 2026-06-20** — den slopade "Min arbetsyta" (personlig ≠ verktygs-launcher). Portfölj-/health-rollup hör i så fall i en **egen** yta, inte i Min arbetsyta. **Arbetsytans innehåll är PARKERAT:** `Workspace` är placeholder; "Dina modeller" ska INTE visa hela katalogen (28 interna system) — öppen fråga om den ska visa produkter/ventures (`productsOnly`) eller ett nytt per-modell "pinnat"-fält i CNS. Auth-placeholder (`src/lib/auth.js`) + **CNS-klient (`src/lib/cns.js`, levande söm — inte längre stub)**. Avsiktligt lätt — bygg inte tunga dashboards här (den tunga grafen är parkerad i `_experiments`).
